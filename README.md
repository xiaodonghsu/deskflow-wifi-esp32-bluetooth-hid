...
屏幕与对端地址的映射、BLE 绑定密钥以及 HID 通知订阅均持久化存储在 NVS 中。ESP32 会持续进行快速可连接广播，直到所有三个槽位均已连接，并在任意目标断开时恢复广播。重启后，已绑定的手机或平板可在发现广播后自动重连；只有链路加密成功且键盘、鼠标通知均恢复后，设备才被视为已连接。若连接后 10 秒内 HID 未就绪，ESP32 会主动断开并重新广播，避免卡在无法收发输入的半连接状态。手机或平板必须保持蓝牙开启，并将该设备保留在已配对设备列表中。
